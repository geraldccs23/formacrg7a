...
export function PurchaseOrders() {
    ...
    const [newProductData, setNewProductData] = useState({ codigo_producto: '', descripcion: '' });
    ...
    const handleCreateProduct = async () => {
        ...
        try {
            ...
            setNewProductData({ codigo_producto: '', descripcion: '' });
        } catch (error: any) {
            console.error(error);
                        ...
                        <div className="p-6 flex-1 overflow-y-auto space-y-6">
                                ...
                                <div className="space-y-2 relative">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input 
                                            ...
                                            value={selectedSupplier ? selectedSupplier.supplier_name : supplierSearch}
                                            onChange={e => {
                                    ...
                                    {showSupplierResults && !selectedSupplier && (
                                        <div className="absolute top-full left-0 right-0 bg-white shadow-2xl border rounded-xl mt-2 z-[110] overflow-hidden max-h-60 overflow-y-auto">
                                            {suppliers
                                                .filter(s => s.supplier_name.toLowerCase().includes(supplierSearch.toLowerCase()) || s.supplier_code.toLowerCase().includes(supplierSearch.toLowerCase()))
                                                .map(s => (
                                                    <button 
                                                        key={s.supplier_code}
                                                        onClick={() => {
                                                    ...
                                                    >
                                                        <div>
                                                            <div className="font-black text-gray-800 text-xs uppercase">{s.supplier_name}</div>
                                                            <div className="text-[10px] text-gray-400 font-bold">{s.supplier_code}</div>
                                                        </div>
                            ...
                            <div className="space-y-4 pt-4 border-t">
                                <div className="flex justify-between items-center gap-4">
                                    <h4 className="font-black text-gray-800 uppercase text-sm tracking-widest whitespace-nowrap">Productos del Pedido</h4>
                                    <div className="flex-1 flex justify-end gap-3">
                                        ...
                                        <button 
                                            ...
                                            className="px-4 py-2.5 bg-gray-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#D40000] transition-all flex items-center gap-2"
                                        >
                                            <Plus size={14} /> Nuevo
                                        </button>
                                    </div>
                ...
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-white/20">
                        <div className="p-6 bg-gray-900 text-white flex justify-between items-center">
                            <h3 className="font-black uppercase tracking-widest text-sm">Crear Nuevo Producto</h3>
                            <button onClick={() => setIsProductModalOpen(false)} className="text-gray-400 hover:text-white transition-colors"><XCircle size={20} /></button>
                        </div>
                        <div className="p-8 space-y-6">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Código del Producto</label>
                                <input 
                                    type="text" 
                                    value={newProductData.codigo_producto}
                                    onChange={e => setNewProductData({...newProductData, codigo_producto: e.target.value.toUpperCase()})}
                            ...
                            <button 
                                ...
                                disabled={!newProductData.codigo_producto || !newProductData.descripcion || creatingProduct}
                            ...
                            >
                                {creatingProduct ? <Loader2 className="animate-spin" size={20} /> : <CheckCircle size={20} />}
